---
title: baseline
description: Manage suppression baselines
---

# hardpoint baseline

Manage suppression baselines for tracking and controlling false positive suppressions.

## Usage

```bash
hardpoint baseline <command> [flags]
```

## Commands

| Command | Description |
| ------- | ----------- |
| `list` | List all suppressions in the current baseline |
| `add` | Add a new suppression to the baseline |
| `prune` | Remove expired or unused suppressions |
| `trust` | Mark the current baseline as trusted |
| `untrust` | Remove trust from the baseline |
| `check` | Check if the baseline is trusted |

## baseline list

Display all suppressions defined in the baseline file.

```bash
hardpoint baseline list [flags]
```

### Flags

| Flag | Short | Description |
| ---- | ----- | ----------- |
| `--expired` | | Only show expired suppressions |

### Examples

```bash
# List all suppressions
hardpoint baseline list

# Show only expired suppressions
hardpoint baseline list --expired
```

### Output

```
Baseline: /Users/dev/.hardpoint/baseline.yaml
Suppressions: 3

  AI-001
    Reason: Intentional override for testing
    File: testdata/**

  SHELL-002
    Reason: Official Homebrew installer
    Pattern: homebrew

  SECRET-001 [EXPIRED]
    Reason: Temporary during migration
    Expires: 2024-12-01T00:00:00Z
```

## baseline add

Add a new suppression to the baseline file.

```bash
hardpoint baseline add <finding-id> [flags]
```

### Flags

| Flag | Short | Description |
| ---- | ----- | ----------- |
| `--reason` | `-r` | Reason for the suppression (required) |
| `--file` | `-f` | File glob pattern to match |
| `--pattern` | `-p` | Regex pattern to match in line content |
| `--expires` | `-e` | Expiration date (ISO 8601 format) |

### Examples

```bash
# Add a basic suppression
hardpoint baseline add AI-001 --reason "Known safe pattern"

# Add suppression for specific files
hardpoint baseline add SECRET-001 --reason "Test fixtures" --file "tests/**/*.env"

# Add suppression with content pattern
hardpoint baseline add SHELL-002 --reason "Official installers" --pattern "brew\.sh|nvm\.sh"

# Add temporary suppression
hardpoint baseline add GIT-001 --reason "Investigating" --expires "2025-03-01"
```

## baseline prune

Remove expired or unused suppressions from the baseline.

```bash
hardpoint baseline prune [flags]
```

### Flags

| Flag | Short | Description |
| ---- | ----- | ----------- |
| `--dry-run` | `-n` | Show what would be removed without modifying |
| `--unused` | | Also remove suppressions that don't match any current findings |

### Examples

```bash
# Remove expired suppressions
hardpoint baseline prune

# Preview what would be removed
hardpoint baseline prune --dry-run

# Remove both expired and unused suppressions
hardpoint baseline prune --unused
```

### Output

```
Pruning baseline: /Users/dev/.hardpoint/baseline.yaml

Removed 2 suppressions:
  - SECRET-001 (expired: 2024-12-01)
  - AI-003 (unused: no matching findings)

Baseline updated successfully.
```

## baseline trust

Mark the current baseline file as trusted.

When you clone a repository that contains a `.hardpoint/baseline.yaml` file, Hardpoint warns you that the baseline may have been crafted to hide malicious findings. Use this command after reviewing the suppressions.

```bash
hardpoint baseline trust [flags]
```

### Examples

```bash
# Review the baseline first
hardpoint baseline list

# Then trust it
hardpoint baseline trust
```

### Output

```
Trusted baseline: /path/to/project/.hardpoint/baseline.yaml
  Suppressions: 5

The baseline will now be used without warnings.
```

## baseline untrust

Remove trust from the baseline file.

```bash
hardpoint baseline untrust [flags]
```

### Examples

```bash
hardpoint baseline untrust
```

### Output

```
Removed trust from baseline: /path/to/project/.hardpoint/baseline.yaml

You will see warnings about this baseline on the next scan.
```

## baseline check

Check if the current baseline file is trusted.

```bash
hardpoint baseline check [flags]
```

### Examples

```bash
hardpoint baseline check
```

### Output (trusted)

```
Baseline is trusted: /path/to/project/.hardpoint/baseline.yaml
  Suppressions: 5
```

### Output (untrusted with high-risk suppressions)

```
WARNING: Untrusted Baseline File Detected
========================================

Path: /path/to/project/.hardpoint/baseline.yaml
Suppressions: 3
Source: Committed to repository (potentially by untrusted source)

High-Risk Suppressed Rules:
  - AI-001
  - AI-003

REVIEW REQUIRED: This baseline suppresses security-critical rules.
Run 'hardpoint baseline list' to inspect before trusting.
```

## Related

- [Baseline & Suppressions](/hardpoint/concepts/baseline) - Understanding suppression configuration
- [scan command](/hardpoint/cli/scan) - Running security scans
