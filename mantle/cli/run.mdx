...
|--------|-------------|---------|
| `-m, --mode <mode>` | Protection mode: redact, block, detect | `redact` |
| `-p, --providers <list>` | Comma-separated providers | `auto` |
| `-s, --scope <scope>` | Environment scope | `development` |
| `-r, --redact-text <text>` | Custom redaction placeholder | `[REDACTED]` |
...
### Explicit Providers
```bash
mantle run --providers dotenv,aws-sm -- node server.js
```
...
Mantle uses a curated set of high-accuracy patterns (AWS keys, GitHub tokens, OpenAI keys, etc.) to scan your output.

See the [Security Policies](/mantle/concepts/security-policies) guide for a full list of supported patterns.
