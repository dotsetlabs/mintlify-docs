...
When you run `dotset mantle run`, Mantle uses a **Secret Resolver** to collect data from a list of configured providers.

```mermaid
graph TD
    A[dotset mantle run] --> B[Secret Resolver]
    B --> C[Dotenv Provider]
    B --> D[AWS Provider]
...
dotset mantle run --providers dotenv,aws-sm -- npm start
```
