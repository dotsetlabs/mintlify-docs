---
title: GitHub Actions
description: 'Protect your GitHub Actions workflows'
---

# GitHub Actions Integration

Mantle integrates seamlessly with GitHub Actions to protect your CI builds.

## Basic Setup

Mantle auto-detects secrets from environment variables injected by GitHub Actions:

```yaml
- name: Install Mantle
  run: npm install -g @dotsetlabs/mantle

- name: Run tests with protection
  run: mantle run --mode redact -- npm test
  env:
    DATABASE_URL: ${{ secrets.DATABASE_URL }}
    API_KEY: ${{ secrets.API_KEY }}
```

## Using with .env Files

If your workflow creates a `.env` file from secrets, Mantle picks it up automatically:

```yaml
- name: Create .env from secrets
  run: |
    echo "DATABASE_URL=${{ secrets.DATABASE_URL }}" >> .env
    echo "API_KEY=${{ secrets.API_KEY }}" >> .env

- name: Run with protection
  run: mantle run --mode redact -- npm test
```

## Using AWS Secrets Manager

Mantle can pull secrets directly from AWS during your build:

```yaml
- name: Run with AWS Secrets
  run: mantle run --providers aws-sm --mode redact -- npm deploy
  env:
    AWS_ACCESS_KEY_ID: ${{ secrets.AWS_ACCESS_KEY_ID }}
    AWS_SECRET_ACCESS_KEY: ${{ secrets.AWS_SECRET_ACCESS_KEY }}
    AWS_REGION: us-east-1
```

## Complete Examples

For comprehensive, real-world GitHub Actions workflow examples, see the **[testing repository](https://github.com/dotsetlabs/testing)**:

- **PR Scanning**: Inline annotations for secret findings
- **SARIF Upload**: Integration with GitHub Code Scanning
- **Build Log Redaction**: Protected vs unprotected build comparison
- **CI Blocking**: Fail builds when secrets are detected

```bash
# Clone and explore the examples
git clone https://github.com/dotsetlabs/testing.git
cd testing/.github/workflows
```

<CardGroup cols={2}>
  <Card title="scan-on-pr.yml" icon="code-pull-request" href="https://github.com/dotsetlabs/testing/blob/main/.github/workflows/scan-on-pr.yml">
    Add inline annotations to PRs
  </Card>
  <Card title="scan-sarif.yml" icon="shield" href="https://github.com/dotsetlabs/testing/blob/main/.github/workflows/scan-sarif.yml">
    Upload to GitHub Code Scanning
  </Card>
  <Card title="build-with-redaction.yml" icon="eye-slash" href="https://github.com/dotsetlabs/testing/blob/main/.github/workflows/build-with-redaction.yml">
    Redact secrets from build logs
  </Card>
  <Card title="block-on-secrets.yml" icon="ban" href="https://github.com/dotsetlabs/testing/blob/main/.github/workflows/block-on-secrets.yml">
    Block PRs with secrets
  </Card>
</CardGroup>
