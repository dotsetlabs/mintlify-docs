...
Replay a captured production error locally.

```bash
dotset replay <eventId> [options]
```

| Option | Description |
|:-------|:------------|
| `--cmd <command>` | Command to start the local server (default: npm start) |
| `--scope <scope>` | Secrets scope to use (default: development) |
| `--port <port>` | Local port to send the replay to (default: 3000) |
| `--delay <ms>` | Delay in ms before replaying (default: 2000) |
