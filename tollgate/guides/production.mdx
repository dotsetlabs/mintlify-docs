...
tollgate wrap ...  # Not recommended

# Use config mode with explicit policies
tollgate start -s postgres -c /etc/tollgate/config.yaml
```
...
tollgate start -s postgres --audit-path /var/log/tollgate/audit.db
```
...
0 0 * * * /usr/local/bin/tollgate export \
...
Type=simple
User=tollgate
ExecStart=/usr/local/bin/npx @dotsetlabs/tollgate tollgate start \
  -s postgres \
...
RUN npm install -g @dotsetlabs/tollgate

COPY tollgate.yaml /etc/tollgate/config.yaml

CMD ["npx", "@dotsetlabs/tollgate", "tollgate", "start", \
     "-s", "postgres", \
     "-c", "/etc/tollgate/config.yaml", \
     "--audit-path", "/var/log/tollgate/audit.db", \
     "--failure-mode", "fail-closed"]
...
```bash
tollgate start -s postgres -c /etc/tollgate/config.yaml &
tollgate start -s filesystem -c /etc/tollgate/config.yaml &
```
