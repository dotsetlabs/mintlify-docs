...
```bash
tollgate validate [path] [options]
```
...
tollgate validate ~/configs/tollgate.yaml
```
...
tollgate validate -s postgres

# Only validate filesystem server
tollgate validate -s filesystem
```
...
tollgate validate --json

# Fail silently except for errors
tollgate validate --quiet
```
...
1. **Always validate before deploying:**
   ```bash
   tollgate validate && tollgate start -s postgres
   ```
...
2. **Add to CI/CD pipeline:**
   ...
   - name: Validate Tollgate config
     run: npx @dotsetlabs/tollgate tollgate validate --quiet
   ```

3. **Use JSON output for programmatic checks:**
   ```bash
   tollgate validate --json | jq '.valid'
   ```
